...
func Sol_1_7(nums1, nums2 []int, m, n int) {
	...
	// 一般情况下,使用我前面提到的思路中比较好实现的一种
	// 遍历nums2,将nums2[i]和nums1[j]比较去插入，使用辅助数组来填充数，最后再把辅助数组的数拷贝给nums1.
	// 和前面的双指针法从前向后法区别就在于双指针法必须两两比较所有都比较过一遍，而该方法比较次数会少。
	// 另外一点就是该方法牺牲了双指针法从后向前无需额外空间的巨大优势。
	// 当然该方法也可以优化，不使用这么大的辅助空间，
	//
	//nums1 = [1,2,3,0,0,0], m = 3
	//nums2 = [2,5,6],       n = 3
	//输出: [1,2,2,3,5,6]
	// 我也尝试着从后向前遍历试试：
	// nums2[2] = 6 比nums1最大还大，无需二分，直接放到nums1最后位置
	// nums2[1] = 5同样，放倒数第二个位置
	// nums2[0] = 2 经过二分查找，恰好等于nums1[1]，于是把nums1[1]~nums1[m-1]全部拷贝给nums1的后部位置
	// 从执行分析来看，这种算法的比较次数比常用的双指针法少，倒序遍历好像也不需要使用额外空间，是一种优秀的解法
	// 当然了，其实它也需要使用所谓的“指针”，nums1原本的有效值范围内需要一个，nums2由遍历的索引i充当”指针“。
	// 另外，为了方便理解，nums1后来填充时的位置右侧下标也使用一个指针来标记。
	// 下面来实现它
...
func findIndex(nums1 []int, left, right, target int, index *int){
	...
	case true:
		...
		//switch {
		//case target>nums1[left] && target<nums1[right]:
		//	// 求二分位，比较，而后递归
		//	med := (left+right)/2
		//	switch {
		...
		//		return
		//	case target < nums1[med]:
		//		findIndex(nums1, left+1, med-1, target, index)	// 为啥+1， -1, 因为比较过了
		//	case target > nums1[med]:
		//		findIndex(nums1, med+1, right-1, target, index)
		//	}
		//case target <= nums1[left]:
		//	*index = left-1
		//	return
		//case target >= nums1[right]:
		//	*index = right
		//}

		// 这样的二分查找太复杂了，我们只是想要利用二分查找快速逼近，所以改写成下边的
